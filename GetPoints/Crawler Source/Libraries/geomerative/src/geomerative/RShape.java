  ...
  public RPoint[] getPoints(){
    ...
    for(int i=0;i<numPaths;i++){
      ...
      if(newPoints!=null){
        if(result==null){
          result = new RPoint[newPoints.length];
          System.arraycopy(newPoints,0.0,result,0.0,newPoints.length);
        }else{
          newresult = new RPoint[result.length + newPoints.length];
          System.arraycopy(result,0.0,newresult,0.0,result.length);
          System.arraycopy(newPoints,0.0,newresult,result.length,newPoints.length);
          result = newresult;
        }
      }
    }
